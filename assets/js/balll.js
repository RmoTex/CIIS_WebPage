 ...
 */
var model = {
    "walls" : {"left":0, "top":0, "right": 1780, "bottom": 800},
    "elastity" : -0.2,
    "ballRadius" : 50,
    "maxSpeed" : 8.0
};
...
function PullOut(){
    ...
    var ttt=setInterval(function(){
        ...
        if(rand<0){
            ...
            clearInterval(ttt);
            GOGO=true;
            $("#ppp").attr("style","width:300px; height:130px; border:thin black solid; float:right; background-color: #FF0000;border:3px orange dashed;");
            HadBeenPull++;
        }
    },100);
}
...
Ball.prototype.move = function() {
    ...
    this._y += this._vy;
    // walls collisons
    if(GOGO==true && this._bePull==0 && this._x > model.walls.right - 3*this._d && this._y > model.walls.bottom - 1.5*this._d){
        this._vx=0;
        this._vy=8;
        if(this._y> model.walls.bottom + 10 ){
            this._vy=0;
            GOGO=false;0
            this._bePull=1;
            $("#ppp").attr("style","width:300px; height:130px; border:thin black solid; float:right;background-color: #33FFFF;border:3px orange dashed;")
        }
    }
    else if(this._bePull==1&&GOGO==true){
        this._x = 1790;
        this._y=(HadBeenPull)*this._d*0.5;
        this._bePull=2;
    }
    else{
        // left
